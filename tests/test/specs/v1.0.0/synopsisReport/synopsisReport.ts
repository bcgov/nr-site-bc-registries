 ...
 * Ticket: N/A
 * Browser: Chrome
 * Purpose: This script tests that a known site generates certain values in its synopsis report.
 * 1. Login on the bc service website.
 * 2. Navigate to site id search.
 * 3. Search with a known site id.
 * 4. Download the synopsis report.
...
const pdfFile: string = downloadDir + `\\synopsis-report_siteid-${siteId}.pdf`;
let pdfText: string;
...
describe('Synopsis Report Test', () => {
  ...
  describe('Login and check the index page', () => {
    ...
    it('Verify email button is displayed', async () => {
      ...
      await viewSearchResultsPage.sitesTableFirstRowReportTypeSelect.selectByIndex(1);
    ...
    it('Check for End of Synopsis Report', async () => {
      assert(pdfText.includes('End of Synopsis Report'));
    });
  });
});
